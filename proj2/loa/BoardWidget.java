...
class BoardWidget extends Pad {
    ...
    private void mousePressed(Square s) {
        ...
        if (p == _board.turn()) {
            ...
            System.out.println(s.toString() + " selected. "
                    + "Please click desired destination.");
            _start = s;
        } else if (p != _board.turn() && _start == null) {
            System.out.println("Please choose your piece.");
        }
        if (p != _board.turn() && s != _start) {
            _dest = s;
        }
        repaint();
    }
    ...
    private void mouseReleased(Square s) {
        Move input = Move.mv(_start, s);
        Move second = Move.mv(_start, _dest);
        boolean legal = _board.legalMoves().contains(input);
        if (legal) {
            _commands.add(input.toString());
            _board.makeMove(input);
            update(_board);
            System.out.println(input.toString());
            _dest = null;
        } else if (!_board.legalMoves().contains(second)
                && _dest != null) {
