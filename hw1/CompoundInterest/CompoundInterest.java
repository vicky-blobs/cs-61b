...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        double rrate = (rate / 100) + 1;
        double futureV = Math.pow(rrate, numYears(targetYear)) * presentValue;
        return futureV;
    }
     ...
     *  compounds annually at a constant rate of INFLATIONRATE.s
     *
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double infrate = 1 - (inflationRate / 100);
        double prev = futureValue(presentValue, rate, targetYear);
        return prev * Math.pow(infrate, numYears(targetYear));
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double infrate = 1 - (inflationRate / 100);
        return totalSavings(perYear, targetYear, rate) * Math.pow(infrate, numYears(targetYear));
    }
